// NOTE: Supabase constraint needs update to allow source='rededor'
// Run: ALTER TABLE savior_bookings DROP CONSTRAINT savior_bookings_source_check;
//      ALTER TABLE savior_bookings ADD CONSTRAINT savior_bookings_source_check CHECK (source IN ('web','whatsapp','phone','api','hospital_system','insurance','corporate_portal','unimed','rededor'));

import { useState, useEffect, useMemo, useRef } from "react";
import { supabase } from "../../lib/supabase";
import './rededor-booking.css';
          ...
          return (
            <button key={h.id} type="button" onClick={() => onChange(h.id)}
              ...
              <div className="bf-hosp-initials"
                style={{ background: net?.color_bg || "#003B71", color: net?.color_text || "#FFFFFF" }}>
...
// ===== MAIN =====
export default function RedeDorBooking() {
  ...
  useEffect(() => {
    if (form.patient_name || form.diagnosis_select) {
      sessionStorage.setItem("rededor_draft", JSON.stringify({ form, step }));
    }
  }, [form, step]);

  // Persistence: restore on load
  useEffect(() => {
    const draft = sessionStorage.getItem("rededor_draft");
    if (draft) {
      try {
  ...
  async function handleSubmit() {
    ...
    sessionStorage.removeItem("rededor_draft");
    setSubmittedMsg(msg);
  ...
  if (loading) return (
    <div className="bf-loading">
      <div className="bf-loading-inner">
        <div className="bf-loading-brand">REDE D'OR</div>
    ...
    <div className="bf-shell">
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <img src="/img/rededor-logo.webp" alt="Rede D'Or" style={{ height: 28, filter: 'brightness(0) invert(1)' }} />
        </div>
          ...
          <div>
            ...
            <Field id="origin" label="Endereço de origem (onde buscar o paciente)" required>
              ...
              {form.origin_hospital_id && (
                <div style={{ marginTop: 6, fontSize: 12, color: '#003B71' }}>
